...
bool Bitmap::drawFile(const char* filename, int x, int y, uint16_t bg_color)
{
	...
	unsigned long initialStartTime = millis(),
		loadTime = 0, drawTime = 0,calcTime = 0;
		...
		for (row = 0; row < min(h, graphics.settings.screenHeight) ; row++) { 
			...
			for (col = 0; col < w; col++) { // For each column...
				...
				case 16:
					hi = sdbuffer[buffidx++];
					lo = sdbuffer[buffidx++];
                    //red 6 bits, green 5 bits, blue 6 bits, shift two bits to get red from hi, then shift 3 to divide from 6 bit to 3 bit.
                    // instead of shifting 5 bits right then 5 bits left, mask with upper 3 bits
                    //green is 2 least significant bits from hi, and 3 most significant from lo. to reduce from 5 bit to 3 bit, we will shift off lowest two
                    // so green is 2 bits from hi and msb from lo
                    //blue is lowest 6 bits. to get to 2 bits, shift right 4
					lcdbuffer[lcdidx++] = (hi & 0xE0 ) | ((((hi&0x3) << 1) | (lo>>7)) << 2) | ((lo &0x3F) >> 4);
					break;
				case 24:
					b = sdbuffer[buffidx++];
					g = sdbuffer[buffidx++];
