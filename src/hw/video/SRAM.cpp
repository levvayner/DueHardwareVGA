...
void SRAM::begin()
{
    ...
    //enable and set as input, diable pullup D22, SCREEN_VISIBLE (active LOW)
...
uint16_t SRAM::ReadShort(uint32_t addr)
{
    ...
    #else        
        //PIOC->PIO_ODR |= (0xFF << 1);
        uint16_t readValue =  (PIOC->PIO_PDSR >> 12) & 0xFF;	            
        readValue |= (PIOB->PIO_PDSR >> 6) & 0xFF00; 
    #endif
    DeviceOff();
	return readValue;
}
